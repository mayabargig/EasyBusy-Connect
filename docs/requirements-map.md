...
This file tracks each requirement so no item is forgotten near the deadline.

| Requirement | Planned implementation | Status |
|---|---|---|
| Social network | Business community, profiles, posts, appointments and chat | Profiles, feed, appointments and chat complete |
| Node.js + Express | REST API in `server` | Stage 1 complete |
| MongoDB | Mongoose models | Stage 1 complete |
| MVC-style separation | Models, controllers, routes and React views | Stage 1 complete |
| At least 3 models | User, Post, Appointment and Message; business details are embedded in User | Complete |
| CRUD through UI | Full screens for each model | Complete for User, Post, Appointment and Message |
